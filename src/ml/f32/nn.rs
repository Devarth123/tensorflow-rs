...
pub fn neural_network<'a>(parameters_: &'a [u16], learning_rate_: &'a f32) -> NeuralNetwork<'a>{
  NeuralNetwork{
      parameters:  parameters_,
      learning_rate: *learning_rate_,
      hidden_weights: matrix::matrix_create(&parameters_[1], &parameters_[0]),
      output_weights: matrix::matrix_create(&parameters_[parameters_.len()-1], &parameters_[1])
  } 
}
pub fn neural_network_train64(nn: &mut NeuralNetwork, input: &matrix::MatrixStruct, output: &matrix::MatrixStruct){
    {
        ...
        let multiplied_matrix = matrix::multiply(&output_errors , &(activation::sigmoid_prime(&final_outputs))); 
        ...
        let multiplied_matrix = matrix::multiply(&hidden_errors, &(activation::sigmoid_prime(&hidden_ouputs)));
