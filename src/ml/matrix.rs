...
pub fn dot32<'a>(m1: &'a MatrixStruct32, m2: MatrixStruct32) ->  MatrixStruct32{
...
pub fn dot64<'a>(m1:  &MatrixStruct64, m2: MatrixStruct64) ->  MatrixStruct64{
        assert_eq!(m1.rows, m2.columns, "the dimensions dont match matrix1.row = {} \t matrix2.column = {}", m1.rows, m2.columns);
...
pub fn add32<'a>(m1: &'a MatrixStruct32, m2: &'a MatrixStruct32) -> MatrixStruct32{
...
pub fn add64<'a>(m1: &'a MatrixStruct64, m2: &'a MatrixStruct64) -> MatrixStruct64{
        assert!(m1.rows == m2.rows && m1.columns == m2.columns, "the dimensions dont match matrix1.rows = {}, matrix1.columns = {}, matrix2.rows = {}, matrix2.columns = {}", m1.rows, m1.columns, m2.rows, m2.columns); 
...
pub fn subtract32<'a>(m1: &'a MatrixStruct32, m2: &MatrixStruct32) -> MatrixStruct32{
        assert!(m1.rows == m2.rows && m1.columns == m2.columns, "the dimensions dont match matrix1.rows = {}, matrix1.columns = {}, matrix2.rows = {}, matrix2.columns = {}", m1.rows, m1.columns, m2.rows, m2.columns); 
...
pub fn subtract64<'a>(m1: &'a MatrixStruct64, m2: &MatrixStruct64) -> MatrixStruct64{
        assert!(m1.rows == m2.rows && m1.columns == m2.columns, "the dimensions dont match matrix1.rows = {}, matrix1.columns = {}, matrix2.rows = {}, matrix2.columns = {}", m1.rows, m1.columns, m2.rows, m2.columns); 
...
pub fn multiply32<'a>(m1: &'a MatrixStruct32, m2: &MatrixStruct32) -> MatrixStruct32{
        assert!(m1.rows == m2.rows && m1.columns == m2.columns, "the dimensions dont match matrix1.rows = {}, matrix1.columns = {}, matrix2.rows = {}, matrix2.columns = {}", m1.rows, m1.columns, m2.rows, m2.columns); 
        let mut matrix = matrix_create32(*m1.rows, *m1.columns);
        for i in 0..*m1.rows as usize{
                for j in 0..*m2.columns as usize {
...
pub fn multiply64<'a>(m1: &'a MatrixStruct64, m2: &MatrixStruct64) -> MatrixStruct64{
        assert!(m1.rows == m2.rows && m1.columns == m2.columns, "the dimensions dont match matrix1.rows = {}, matrix1.columns = {}, matrix2.rows = {}, matrix2.columns = {}", m1.rows, m1.columns, m2.rows, m2.columns); 
        let mut matrix = matrix_create64(*m1.rows, *m1.columns);
        for i in 0..*m1.rows as usize{
                for j in 0..*m2.columns as usize {
...
pub fn add_by_scalar32<'a>(m1: &'a MatrixStruct32, num: &f32) -> MatrixStruct32 {
       let mut matrix = matrix_create32(*m1.rows, *m1.columns);
       for i in 0..*m1.rows as usize{
               for j in 0..*m1.columns as usize{
...
pub fn add_by_scalar64<'a>(m1: &'a MatrixStruct64, num: &f64) -> MatrixStruct64 {
       let mut matrix = matrix_create64(*m1.rows, *m1.columns);
       for i in 0..*m1.rows as usize{
               for j in 0..*m1.columns as usize{
                       matrix.matrix[i][j] += *num;
...
pub fn multiply_by_scalar32<'a>(m: &'a MatrixStruct32, num: &f32) -> MatrixStruct32{
...
pub fn multiply_by_scalar64<'a>(m: &'a MatrixStruct64, num: &f64) -> MatrixStruct64{
...
pub fn transpose32<'a>(m: &'a MatrixStruct32) -> MatrixStruct32{
...
pub fn transpose64<'a>(m: &'a MatrixStruct64) -> MatrixStruct64{
...
pub fn copy_matrix<'a>(m: &'a MatrixStruct64) -> MatrixStruct64{
...
pub fn scale<'a>(num:  &'a f64, m1: &'a MatrixStruct64) -> MatrixStruct64{
